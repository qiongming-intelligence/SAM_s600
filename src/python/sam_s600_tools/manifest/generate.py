...
def write_include_manifest(path: Path, name: str, includes: list[str]) -> None:
    lines = [f"name: {name}", "version: pre-alpha", "include:"]
...
def main() -> int:
    ...
    parser.add_argument("--out-dir", type=Path, default=Path("configs/manifests"))
    ...
    write_include_manifest(args.out_dir / "sam3_full.yaml", "sam3_full", ["sam3_image.yaml", "sam3_video.yaml", "sam3_multiplex.yaml"])
    return 0
