...
The current pre-alpha CLI validates request wiring and model manifests. The C++ `Sam3VideoPredictor` validates required SAM3 video partitions (image encoder, detector, mask decoder, memory encoder, tracker) and runs the image encoder on each raw frame whose bytes already match the converted HBM input tensors. The C++ `Sam3MultiplexVideoPredictor` mirrors this for SAM3.1 by validating and chaining `multiplex_detector`, `multiplex_tracker`, and `memory_encoder` partitions. Tracker/memory chaining for non-multiplex video, demuxing, and full multiplex object-ID propagation will land in following stages.
